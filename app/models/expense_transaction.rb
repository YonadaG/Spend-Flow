class ExpenseTransaction < ApplicationRecord
  ...
  validates :direction, inclusion: { in: %w[debit credit], message: "%{value} is not a valid direction", allow_nil: true }, allow_blank: true
  validates :currency, inclusion: { in: %w[ETB USD EUR GBP KES], allow_nil: true }, if: -> { currency.present? }
